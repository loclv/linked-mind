...
- Mind-Map RAG: For long Markdown documents, run li mind build doc.md to structure them as concept mind-maps (section tree + causal links). The system extracts headings, builds a hierarchy, and serializes to mind-map.json. Query via li mind query "What causes X?" which loads the mind-map, traverses leaf nodes, assembles context, and sends a prompt to an OpenAI-compatible LLM. Set OPENAI_API_KEY to enable. The LLM service uses a native Zig HTTP client (std.http.Client) with no external dependencies (no vectors, no chunking, purely reasoning-based), now enhanced with HTTP request retry with exponential backoff and model fallback support.
## 🛠 Advanced Prompting
